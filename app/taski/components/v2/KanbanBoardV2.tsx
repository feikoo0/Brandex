...
import { TaskCardV2Content } from "./TaskCardV2";
import KanbanColumnV2, { SynthesizedTask } from "./KanbanColumnV2";
...
export const KanbanBoardV2: React.FC<KanbanBoardV2Props> = ({
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const projectId = activeTask.projectId ?? (activeId.startsWith("kt-") ? activeId.split("-")[1] : 0);
    const taskNum = activeId.startsWith("kt-") ? activeId.split("-").slice(2).join("-") : activeId;

    let oldColId: string | undefined = undefined;
    const project = projects.find((p) => String(p.id) === String(projectId));
    const originalTask = project?.tasks?.find(
      (t) => String(t.id) === String(taskNum) || (project ? `kt-${project.id}-${t.id}` === activeId : false)
    );
    if (originalTask) {
      if (groupingMode === "estado") {
